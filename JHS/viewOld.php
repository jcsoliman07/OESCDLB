<?php
	...
	if (isset($_GET['id'])) {
		$update_id = $_GET['id'];
		$sql_update = mysqli_query($conn, "UPDATE `tbl_jhs_oldstudent` SET `notif`=1 WHERE `id`='$update_id'");
	}
?>
...
<?php 
$sql = mysqli_query($conn, "SELECT * FROM tbl_jhs_oldstudent WHERE id='$update_id'");
	...
	<div class="container-fluid">
		...
		<div class="col-sm-12">
			...
			<form method="post" action="viewOldSave.php" autocomplete="off">
				...
				<?php
					$Id = $result['id'];
					$query = mysqli_query($conn, "SELECT * FROM `tbl_jhs_oldstudent` WHERE id='$Id'")or die(mysqli_error($conn));
