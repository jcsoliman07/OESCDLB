		...
		if (mysqli_num_rows($result) > 0) {
			...
			$query = mysqli_query($conn, "SELECT * FROM tbl_login_acc") or die(mysqli_error($conn));
				...
				while ($fetch = mysqli_fetch_array($query)) {
					...
					if ($fetch['login_id'] == $row['student_id']) {
						echo"<script type='text/javascript'>alert('Student already have an account!');window.location.href='index.php'</script>";
					}
				}
		}
		else{
		...
		<div class="col-sm-10">
			...
			<form method="post" action="save.php">
				<div class="form-group row">
					...
					<div class="col-sm-3">
						<input type="text" name="student_id" id="student_id" class="form-control  form-control-sm" value="<?php echo $row['student_id']?>" required>
					</div>
				</div>
				<div class="form-group row">
					<div class="col-sm-2">
				...
				<div class="form-group row">
					...
					<div class="col-sm-3">
						<input type="text" name="name" id="username" class="form-control  form-control-sm" value="<?php echo $row['fname']." ".$row['lname']." ".$row['mname']?>">
					</div>
				</div>
				<br/>
				<div class="form-group row">
					...
					<div class="col-sm-3" style="font-weight: bold;">
						<input type="text" name="username" id="username" class="form-control  form-control-sm" value="<?php echo $row['student_id']?>">
					</div>
				</div>
				<div class="form-group row">
					<div class="col-sm-2">
