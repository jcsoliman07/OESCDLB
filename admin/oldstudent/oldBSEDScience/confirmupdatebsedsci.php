	...
	if(isset($_POST['Add'])){
		...
		if (mysqli_num_rows($result) > 0) {
			while($row = mysqli_fetch_assoc($result)){
				echo "<script type='text/javascript'>alert('Student ID is already taken!');window.location.href='newbsedsci.php'</script>";
			}
		}
		else{
			...
			if ($result) 
			{
				header("Location:newbsedsci.php?success=Successfull!");
			}
			else
			{
				header("Location:newbsedsci.php?error=Unsuccessfully!");
			}	
		}
		mysqli_close($conn);
	}
