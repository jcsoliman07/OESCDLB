					...
					$query = mysqli_query($conn, "SELECT * FROM tbl_instructor") or die(mysqli_error($conn));
						...
						while ($fetch1 = mysqli_fetch_array($query1)) {
								...
								if ($fetch1['subj_code'] == $row['subj_code'] AND $fetch['ins_id'] == $row['instructor_id']) {
									echo"<script type='text/javascript'>alert('Instructor is Already Assigned in this subject!');window.location.href='index.php'</script>";
								}
							}
						}
				}else{
						$query2 = "INSERT INTO `tbl_instructor_subjects`(`instructor_id`, `subj_id`, `subj_code`, `subj_description`, `subj_unit`) VALUES ('$ins_id', '$subj_id', '$subj_code', '$subj_description', '$subj_unit')";
						$result1 = mysqli_query($conn,$query2);

						if ($result1) 
						{
							echo"<script type='text/javascript'>alert('Successfully Assigned!');window.location.href='index.php'</script>";
						}
						else
						{
							echo"<script type='text/javascript'>alert('Unsuccessfully Assigned!');window.location.href='index.php'</script>";
						}
				}
			}
		}
