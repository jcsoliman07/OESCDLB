<?php
	...
	if(isset($_POST['Add'])){
		...
		// $query = mysqli_query($conn, "SELECT student_id FROM tbl_approvalstudent") or die(mysqli_error($conn));
		// $row = mysqli_fetch_assoc($query);
		// $student_id = $row['student_id'];

		$query = "SELECT student_id FROM tbl_approvalstudent";
		$result = mysqli_query($conn, $query);

		if (mysqli_num_rows($result) > 0) {
			while($row = mysqli_fetch_assoc($result)){
				echo "<script type='text/javascript'>alert('Student ID is already taken!');window.location.href='newab.php'</script>";
			}
		}

		// if ($row['student_id'] == $student_id) {
		// 	echo "<script type='text/javascript'>alert('Student ID is already taken!');window.location.href='confirmupdateab.php'</script>";
		// }
		else{
			...
			if ($result) 
			{
				header("Location:newab.php?success=Successfull!");
			}
			else
			{
				header("Location:confirmupdateab.php?error=Unsuccessfully!");
			}	
		}
		mysqli_close($conn);
	...
	{
			header("Location:newab.php?error=Unsuccessfully!");
	}
?>