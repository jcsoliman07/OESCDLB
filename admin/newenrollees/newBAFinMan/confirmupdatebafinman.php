	...
	if(isset($_POST['Add'])){
		...
		if (mysqli_num_rows($result) > 0) {
			while($row = mysqli_fetch_assoc($result)){
				echo "<script type='text/javascript'>alert('Student ID is already taken!');window.location.href='newbafinman.php'</script>";
			}
		}
		else{
			...
			if ($result) 
			{
				header("Location:newbafinman.php?success=Successfull!");
			}
			else
			{
				header("Location:newbafinman.php?error=Unsuccessfully!");
			}	
		}
		mysqli_close($conn);
