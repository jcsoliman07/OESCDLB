	...
	if (isset($_POST['Save'])) {
		...
		for ($i=0; $i <= $counter ; $i++) { 
			...
			if (!empty($_POST["subj_id"][$i])) {
				...
				if($sql){
   					header("Location: approvalbamarketing.php?success=Successfully Added Subject/s");
   				}
   				else
   				{
   					header("Location: approvalbamarketing.php?error=Unsuccessfully Added Subject/s");
   				}
