	       					...
	       					while($row = mysqli_fetch_array($q)){
	       						if ($row['sem_set'] == 1) {
	       							if ($row['semester'] == $sem) {
			       						...
			       						$l = mysqli_query($conn, "SELECT * FROM `tbl_subjects` WHERE  `subj_semester`='$semester' AND `subj_year`='$year' AND `subj_course`='$course'")or die(mysqli_error($conn));

			       						while($fetch = mysqli_fetch_array($l)){
			       							$i++;

			       						$query1 = mysqli_query($conn, "SELECT * FROM tbl_grading");
			       						while ($result1 = mysqli_fetch_array($query1)) {
			       							if ($result1['subj_grade'] == 'Failed') {
			       								// code...
			       							}
			       						}
	       			?>
