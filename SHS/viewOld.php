<?php
	...
	if (isset($_GET['id'])) {
		$update_id = $_GET['id'];
		$sql_update = mysqli_query($conn, "UPDATE `tbl_shs_oldstudent` SET `notif`=1 WHERE `id`='$update_id'");
	}
?>
...
<?php 
$sql = mysqli_query($conn, "SELECT * FROM tbl_shs_oldstudent WHERE id='$update_id'");
while($result = mysqli_fetch_assoc($sql)):?>
	...
	<div class="container-fluid">
		...
		<div class="col-sm-12">
			...
			<form method="post" action="viewOldSave.php" autocomplete="off">
				<div class="form-group row">
					<div class="col-sm-12">
						<button onclick="checker()" name="button" style="display: block;margin-left: auto;margin-right: 0; font-weight: bold;" class="btn btn-info my-5"> Approved <span class="fas far fa-check"></button>
					</div>
				</div>
				<div class="form-group row">
					<div class="col-sm-3">
						<label class="form-label">Student ID: </label>
						<input type="text" name="student_id" class="form-control" placeholder="00-0000" value="<?php echo $result['student_id']?>" readonly>
				...
				<div class="form-group row">
					<div class="col-sm-3">
						...
						<input type="hidden" name="grade" class="form-control" value="<?=$result['grade']?>" readonly>
						<input type="hidden" name="student_status" class="form-control" value="<?=$result['student_status']?>" readonly>
						<input type="hidden" name="strand" class="form-control" value="<?=$result['strand']?>" readonly>
				...
				<?php
					$Id = $result['id'];
					$query = mysqli_query($conn, "SELECT * FROM `tbl_shs_oldstudent` WHERE id='$Id'")or die(mysqli_error($conn));
