<?php
	...
	if (isset($_GET['id'])) {
		$update_id = $_GET['id'];
		$sql_update = mysqli_query($conn, "UPDATE `tbl_oldstudent` SET `notif`=1 WHERE `id`='$update_id'");
	}
?>
...
<body>
	...
	function validate($data){
		...
		$sql = "SELECT * FROM tbl_newstudent WHERE id=$Id";
		...
		if (mysqli_num_rows($result) > 0) {
			...
			header("Location:../admin/newenrollees/newCS/newcs.php");
		}
}else
{
	header("Location:../admin/newenrollees/newCS/newcs.php");
...
<?php 
$sql = mysqli_query($conn, "SELECT * FROM tbl_oldstudent WHERE notif=1");
while($result = mysqli_fetch_assoc($sql)):?>
	...
	<div class="container-fluid">
		...
		<div class="col-sm-12">
			...
			<form method="post" action="viewSaveOld.php" autocomplete="off">
				...
				<div class="form-group row">
					<div class="col-sm-3">
						...
						<input type="text" name="student_id" class="form-control" placeholder="00-0000" value="<?=$result['student_id']?>" required>
				...
				<div class="form-group row">
					<div class="col-sm-3">
						...
						<input type="hidden" name="semester" class="form-control" value="<?=$result['semester']?>" readonly>
						...
						<input type="hidden" name="ayear" class="form-control" value="<?=$result['academic_year']?>" readonly>
					</div>
					<div class="col-sm-3">
				...
				<?php
					$Id = $result['id'];
					$query = mysqli_query($conn, "SELECT * FROM `tbl_newstudent` WHERE id='$Id'")or die(mysqli_error($conn));
