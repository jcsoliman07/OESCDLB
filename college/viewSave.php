<?php
	...
	if (isset($_POST['button'])) {
		...
		$query = mysqli_query($conn,"SELECT student_id FROM tbl_student");
		$result = mysqli_fetch_assoc($query);

		$myquery = mysqli_query($conn,"SELECT student_id FROM tbl_approvalstudent");
		$myresult = mysqli_fetch_assoc($myquery);

		if ($result['student_id'] == $student_id) {
				echo "<script type='text/javascript'>alert('Student ID is already taken and Enrolled!');window.location.href='../admin/newenrollees/newCS/newcs.php'</script>";
		}else if($myresult['student_id'] == $student_id){
			echo "<script type='text/javascript'>alert('Student ID is already taken and in Approval!');window.location.href='../admin/newenrollees/newCS/newcs.php'</script>";
		}else{
				...
				if ($result) {
					...
					$query1 = "DELETE FROM tbl_newstudent WHERE id = '$Id'";
					$result1 = mysqli_query($conn, $query1);

					if ($result1) {
						echo "<script type='text/javascript'>alert('Successfully Save!');window.location.href='../admin/newenrollees/newAB/newab.php'</script>";
					}else{
						echo "<script type='text/javascript'>alert('Unsuccessfully Save!');window.location.href='../admin/newenrollees/newAB/newab.php'</script>";
					}
